...
{foreach from=$ejesL item=item key=key}
	<tr>       
        ...
        <input class="smallInput" name="letter[{$key}]" id="" type="text" size="50" value="{$item.letra|replace:"1":"'"}" style="width:100px"/>
        <input type="hidden" name="idLetter[{$key}]" value="{$item.id}" />
        </td>       
        <td align="center" bgcolor="#FFFFFF">
