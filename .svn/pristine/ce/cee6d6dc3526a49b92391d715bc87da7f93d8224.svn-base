...
{foreach from=$mtoEquip item=item key=key}
	<tr>       
        ...
        <input class="smallInput small2" name="qtyEquip[{$key}]" id="" type="text" size="50" value="{$item.quantity}" onblur="SaveMontoEquip()" />
        <input type="hidden" name="idProjEquip[{$key}]" value="{$item.projEquipId}" />
        </td>
        <td align="center" bgcolor="#FFFFFF">
        {include file="{$DOC_ROOT}/templates/lists/enumCurrencyEquip.tpl"}
        </td>      
        <td align="center" bgcolor="#FFFFFF">
        <a href="javascript:void(0)" onclick="DelMontoEquip({$key})">
