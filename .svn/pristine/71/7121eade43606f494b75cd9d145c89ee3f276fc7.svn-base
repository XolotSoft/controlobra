<form name="frmSearch" id="frmSearch" method="post" action="">
<input type="hidden" name="action" id="action" value="searchResumen" />
<table width="700" cellpadding="0" cellspacing="0" class="tblNoBorder">
<tr>
    <td align="center"><b>Partida</b></td>
    <td align="center"><b>Subpartida</b></td>
    <td align="center"><b>Concepto</b></td>
    <td align="center"><b>Cuenta Bancaria</b></td>
    <td align="center"><b>Con Iva</b></td>
</tr>
<tr>
    <td align="center">{include file="lists/enumCategory2.tpl"}</td>
    <td align="center">
        <div id="enumSubcat">{include file="lists/enumSubcategory2.tpl"}</div>
    </td>
    <td align="center">
        <div id="enumConcept">{include file="lists/enumConceptCon2.tpl"}</div>
    </td>
    <td align="center">
        <div id="enumConcept">{include file="lists/enumBank.tpl"}</div>
    </td>    
    <td align="center">
        <select class="smallInput" name="conIva" id="conIva">
        <option value="1">S&iacute;</option>
        <option value="0">No</option>
        </select>
    </td>
</tr>
<tr><td colspan="5" height="10"></td></tr>
<tr>
    <td align="center" colspan="5">
    <input type="button" class="btnGral" value="Buscar" onclick="SearchConcept()" />
    </td>
</tr>
</table>
</form>