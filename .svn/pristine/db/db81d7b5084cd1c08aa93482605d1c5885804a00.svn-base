...
{foreach from=$mtoMant item=item key=key}
	<tr>       
        ...
        <input class="smallInput small2" name="qtyMant[{$key}]" id="" type="text" size="50" value="{$item.quantity}"/>
        <input type="hidden" name="idQtyMant[{$key}]" value="{$item.id}" />
        </td>
        <td align="center" bgcolor="#FFFFFF">
        {include file="{$DOC_ROOT}/templates/lists/enumCurrencyMant.tpl"}
        </td>  
        <td align="center" bgcolor="#FFFFFF">
        <a href="javascript:void(0)" onclick="DelMant({$key})">
