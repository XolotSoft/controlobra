...
{foreach from=$ejes.N item=item key=key}
	<tr>       
        ...
        <input class="smallInput small2" name="number[{$key}]" id="" type="text" size="50" value="{$item.numero|replace:"A":"'"}"/>
        <input type="hidden" name="idProjEjeN[{$key}]" value="{$item.projEjeNId}">
        </td>       
        <td align="center" bgcolor="#FFFFFF">
