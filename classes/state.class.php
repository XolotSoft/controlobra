...
class State extends Main
{
	...
	{		
		...
		$this->Util()->DB()->setQuery('SELECT COUNT(*) FROM state');
		$total = $this->Util()->DB()->GetSingle();

		$pages = $this->Util->HandleMultipages($this->page, $total ,WEB_ROOT.'/state');

		$sql_add = 'LIMIT '.$pages['start'].', '.$pages['items_per_page'];
		$this->Util()->DB()->setQuery('SELECT * FROM state ORDER BY name ASC '.$sql_add);
		$result = $this->Util()->DB()->GetResult();
	...
	public function Save(){
		...
		$this->Util()->setError(10078, 'complete');
	...
	public function Update(){
		...
		$this->Util()->setError(10079, 'complete');
		$this->Util()->PrintErrors();
	...
	public function Delete(){
		...
		$this->Util()->setError(10080, 'complete');
		$this->Util()->PrintErrors();
